    ...
    if dirFile.endswith("DOB"):
        ...
        for txtFile in allFiles:
            ...
            if txtFile.endswith(".txt"):
                ...
                #         Stim_line.append(line_floats)
                # Stimulus = np.array(Stim_line)

                # BUT Pandas is even faster!
                # Signal = pd.read_csv(fnames, sep='\t', lineterminator='\n', header=None, quoting=2).to_numpy()[:, 1:]
                # Stimulus = pd.read_csv(snames, sep='\t', lineterminator='\n', header=None, quoting=2).to_numpy()
                ...
                if np.count_nonzero(Stim_types_fr_sorted == 3) == Stim_type_amount & np.count_nonzero(
                    ...
                    # Not really faster....sorry ;D
                    # decimal_precision = 4
                    # d = zscore[:, cell_response_index]
                    # with open(f'{file_export_path}_zscore_new.txt', 'w') as f:
                    #     fmt = ' '.join([f'%.{decimal_precision}e'] * d.shape[1])
                    #     fmt = '\n'.join([fmt] * d.shape[0])
                    #     data = fmt % tuple(d.ravel())
                    #     f.write(data)

                    # ++ Nisse ++ By default the code below gives a decimal precision of 18 digits.
                    # Is this really needed?
                    # More over the format is in exponential notation. This will also take more disc space, since the
                    # values are not really large or small

                    # np.savetxt(dir_name + 'results/' + txtFile[:-4] + '_Scores.txt',np.transpose(Scores_mean[:,cell_response_index]))
                    # np.savetxt(dir_name + 'results/' + txtFile[:-4] + '_cutouts.txt',np.reshape(cutouts[:,cell_response_index,:],(cutouts.shape[0]*len(cell_response_index),time_window)))
                    # np.savetxt(dir_name + 'results/' + txtFile[:-4] + '_cell_response_index.txt',cell_response_index)
                    # np.savetxt(dir_name + 'results/' + txtFile[:-4] + '_zscore.txt',zscore[:,cell_response_index])
